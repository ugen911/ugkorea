...
def load_foranalitics_data(engine):
    ...
    prodaja_df = (
        prodaja_df.groupby(['kod', 'period', 'hozoperatsija', 'pokupatel', 'dokumentprodazhi', 'skladkompanii'], as_index=False)
    ...
    prodaja_df = prodaja_df[~((prodaja_df['kod'] == '00000003') & (prodaja_df['hozoperatsija'] == 'Заказ-наряд'))]
    ...
    zakaz_narad_tovar_df["kolichestvo"] = zakaz_narad_tovar_df["kolichestvo"].astype(float)
    ...
    vidremonta_mapping = prodaja_df.dropna(subset=['vidremonta']).set_index('dokumentprodazhi')['vidremonta'].to_dict()

    # Обновляем vidremonta для строк, где rs_zakaznarjad не пустой, а vidremonta пустой
    prodaja_df['vidremonta'] = prodaja_df.apply(
        lambda row: vidremonta_mapping.get(row['rs_zakaznarjad'], row['vidremonta'])
        if pd.notna(row['rs_zakaznarjad']) and pd.isna(row['vidremonta']) else row['vidremonta'],
        axis=1
    )
    # Сортировка по 'period' и 'dokumentprodazhi'
    prodaja_df = prodaja_df.sort_values(by=['period', 'dokumentprodazhi']).reset_index(drop=True)
...
_, _, prodaja_df, _ = load_foranalitics_data(engine)
