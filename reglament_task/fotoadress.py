...
def process_catalog_files():
    ...
    try:
        ...
        df = df.dropna()
        df["photos"] = (
            df["photos"].str.lower().replace({".jpg": "_big.JPG"}, regex=True)
        )
        ...
        df[["kod", "adress"]].to_sql(
            "photoadress", con=engine, if_exists="replace", index=False
        )
...
def get_and_process_data():
    ...
    # Загрузка файла catalog из папки Downloads
    folder_path = os.path.join(os.path.expanduser("~"), "Downloads")
    catalog_files = [
        f for f in os.listdir(folder_path) if re.match(r"catalog(\s\(\d+\))?\.csv", f)
    ]

    if catalog_files:
        # Сортируем файлы по числу в скобках и берем последний
        catalog_files.sort(
            key=lambda f: (
                int(re.search(r"\((\d+)\)", f).group(1))
                if re.search(r"\((\d+)\)", f)
        ...
        catalog_file_path = os.path.join(folder_path, catalog_files[0])

        # Загружаем и очищаем файл catalog
        catalog_df = pd.read_csv(catalog_file_path)
        catalog_df["sku"] = catalog_df["sku"].str.strip()  # Удаление пробелов
        catalog_df.rename(columns={"sku": "kod"}, inplace=True)

        # Фильтруем основной датафрейм по наличию kod в catalog
        kod_catalog = catalog_df["kod"].unique()
    else:
        print("Файл catalog не найден. Продолжаем без фильтрации по catalog.")
        kod_catalog = []
    ...
    filtered_df = filtered_df[
        ...
        & ~filtered_df["naimenovanie"].str.contains(
            "|".join(exclude_contains), case=False, na=False
        )
    ]

    # Оставляем только те строки, где kod есть в catalog, если catalog был загружен
    if len(kod_catalog) > 0:
        filtered_df = filtered_df[filtered_df["kod"].isin(kod_catalog)]
