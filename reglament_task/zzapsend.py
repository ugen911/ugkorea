...
df_final["Б/У"] = df_final["naimenovanie"].apply(
    lambda x: "Б/У" if "б/у" in x.lower() else "Новый"
)

# Замена значений в колонке osnsklad и создание новой колонки "Наличие"
df_final["Наличие"] = df_final["osnsklad"].apply(
    lambda x: "В наличии" if x > 0 else "Отсутствует"
)
...
df_final.drop(columns=["kod"], inplace=True)
...
xlsx_path = os.path.join(output_dir, "Прайс_zzap_обновленные_с_фото.xlsx")
df_final.to_excel(xlsx_path, index=False)

# Объявляем список адресов электронной почты
to_emails = ["price@zzap.pro"]
subject = "Прайс_zzap_обновленные_с_фото"
